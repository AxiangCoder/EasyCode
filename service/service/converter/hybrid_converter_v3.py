...
DSL_OUTPUT_FILE = os.path.join(
    os.path.dirname(__file__), "..", "media", "sketches", "dsl_output_v3.json"
)
...
LLM_API_KEY = "sk-50f4df8af3ae485cb5389f913c287606"  # 在此填入你的 OpenAI API Key
    ...
    if (fills := style.get("fills")) and fills and fills[0].get("isEnabled"):
        ...
        if hex_color:
            if token := token_maps.get("colors", {}).get(hex_color):
                ...
                print(
                    f"[WARNING] 发现未令牌化的颜色: {hex_color} (应用于图层: '{layer_name}')"
                )
    ...
    if text_style := style.get("textStyle"):
        ...
        if font_name and font_size:
            ...
            if token := token_maps.get("fonts", {}).get(font_key):
                ...
                print(
                    f"[WARNING] 发现未令牌化的字体: '{font_key}' (应用于图层: '{layer_name}')"
                )
...
def main():
    """主函数 (V3)"""
    print("--- Sketch-to-DSL Converter V3 (Fixed) ---")
...
def analyze_layout_with_llm(layers):
    ...
    try:
        ...
        client = OpenAI(api_key=LLM_API_KEY, base_url="http://127.0.0.1:1234/v1/chat/completions",)
        simplified_layers = [
