...
DSL_OUTPUT_FILE = os.path.join(os.path.dirname(__file__), '..', 'media', 'sketches', 'dsl_output_v3.json')
...
def traverse_layer(layer, symbol_map, token_maps, report):
    ...
    if layer_class == 'symbolInstance':
        ...
        node.update({'type': 'Group', 'name': layer.get('name'), 'frame': layer.get('frame')})
...
def main():
    """主函数 (V3)"""
    print("--- Sketch-to-DSL Converter V3 (Guardian Mode) ---")
    token_maps = load_design_tokens()
    report = {"unknown_colors": {}, "unknown_fonts": {}}
    ...
    try:
        ...
        print(f"成功！V3 版本的 DSL 文件已生成于 {DSL_OUTPUT_FILE}")
...
def analyze_layout_with_llm(layers):
    ...
    try:
        ...
        response = client.chat.completions.create(model="gpt-3.5-turbo", messages=[{"role": "system", "content": "You are an expert assistant that analyzes UI layouts. Your only output should be a single, clean JSON object."}, {"role": "user", "content": prompt}], temperature=0.1, response_format={"type": "json_object"})
        layout_info = json.loads(response.choices[0].message.content)
        print(f"[INFO] OpenAI 分析成功: {layout_info}")
        return layout_info
    except Exception as e: print(f"[ERROR] OpenAI API 调用失败: {e}"); return {'type': 'absolute'}
