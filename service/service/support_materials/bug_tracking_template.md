...
| Bug-004 | `router.path` 值异常 | 2025-09-29 | S0 阻塞 | P0 | 处理中 | 1. 生成代码<br>2. 检查生成的 `router.path`<br>3. 与预期不符 | `router.path` 与页面配置一致 | `router.path` 错误 |  |  |
| Bug-005 | `router.tsx` 语法错误 | 2025-09-29 | S0 阻塞 | P0 | 处理中 | 1. 生成路由文件<br>2. 打开 `router.tsx`<br>3. 编译时报语法错误 | `router.tsx` 符合语法规范 | 存在语法错误导致编译失败 |  |  |
