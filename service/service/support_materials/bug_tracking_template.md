...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ***SKETCH-1*** | Flex 布局缺少对齐与分布属性 (justify-content / align-items) | 当前的 LLM 布局分析未能稳定返回 `justify-content` 和 `align-items` 这两个关键的 flexbox 属性，导致前端渲染时无法精确控制对齐方式。父节点的 `layout` 对象中应包含 `justifyContent` 和 `alignItems` 等属性，但目前解析器未实现此功能。 | 2025-10-13 | S2 一般 | P1 | 已关闭 | 1. 运行转换任务<br>2. 检查DSL中非Virtual Group的Flex容器 | 所有的Flex容器都应有alignItems和justifyContent属性 | 只有Virtual Group才有对齐属性 | 通过重构布局分析策略，将通用分析逻辑（如对齐、间距计算）提取到独立的 `analysis_utils.py` 模块。新的 `HybridExpertArbitrationStrategy` 现在会为所有推断出的Flex容器（包括顶层容器和虚拟组）统一调用 `analyze_alignment` 函数，确保对齐属性被正确计算和添加。 | | 所有Flex容器都包含正确的对齐属性 | 2025-10-13 |
...
| ***SKETCH-25*** | 代码生成器未实现画板节点的 Flex 布局 | 根据新的 DSL 规范，画板（页面级组件）节点现在可以拥有 `layout: { type: 'flex', ... }` 属性。然而，代码生成阶段并未适配此改动，导致最终生成的代码中缺少对应的 `display: flex` 等 CSS 样式，使得 DSL 中的流式布局设计失效。 | 2025-10-11 | S1 严重 | P0 | 已关闭 | 1. 使用修复后的解析器生成一份包含 `flex` 布局画板的 DSL。<br>2. 使用该 DSL 生成前端代码。<br>3. 检查画板组件对应的 CSS 样式。 | 画板组件的 div 应包含 `display: flex` 样式。 | 生成的样式中缺少 `display: flex`。 | 重构 `FrontendRenderer`，移除了对根节点布局分析的限制(`analyze_layout = not is_root`)，使其能够正确处理并应用页面级组件的 `flex` 或 `grid` 布局属性。 | | 页面级组件能正确应用Flex/Grid布局 | 2025-10-13 |
| ***SKETCH-26*** | 页面级组件布局处理缺陷 | 页面级组件在 DSL 生成过程中存在两个问题：1) 被特殊处理跳过布局分析，导致其 `layout` 字段为空对象 `{}`，无法利用 flex、grid 等布局类型；2) 即使识别为 flex 布局，仍然被强制加上 `position: "absolute"`，导致布局逻辑矛盾和响应式能力丧失。这限制了页面级组件的布局能力，特别是在多画板场景下无法实现响应式布局。 | 2025-01-27 | S1 严重 | P0 | 已关闭 | 1. 使用包含多个画板的 Sketch 文件进行转换<br>2. 检查生成的 DSL 中页面级组件的 layout 字段<br>3. 发现页面级组件的 layout 为空或包含矛盾的布局属性 | 页面级组件应能利用 layout.type 属性支持 flex、grid 等布局，且不应被强制绝对定位 | 页面级组件的 layout 字段为空或包含矛盾的布局属性 | 在 `SketchParser` 的 `run` 方法中增加了后处理步骤，通过遍历DSL根节点的直接子节点，强制移除了页面级组件（顶层画板）上由旧逻辑错误添加的绝对定位。 | 页面级组件能够根据布局类型智能选择定位方式，支持多画板的响应式布局，避免布局逻辑矛盾 | 页面级组件的布局在DSL中正确表达，且不含错误的绝对定位 | 2025-10-13 |
...
| ***SKETCH-28*** | 多个画板被错误合并成单个页面 | 布局引擎在处理顶层多个画板时，会创建一个 `Virtual Group` 将它们包裹起来，导致渲染器只识别出一个页面，而不是多个独立的页面。 | 2025-10-13 | S0 阻塞 | P0 | 已关闭 | 1. 使用包含多个画板的Sketch文件进行转换。<br>2. 检查生成的代码。 | 应生成多个页面文件和路由。 | 只生成了一个页面文件和路由。 | 修改 `SketchParser` 的 `_traverse_layer` 方法，为根 `page` 节点添加特殊处理逻辑，阻止其在顶层创建 `Virtual Group`，确保每个画板都作为独立的子节点输出到 DSL 中。 | | 每个画板都正确生成独立的页面和路由 | 2025-10-13 |
| ***SKETCH-29*** | LLM模型布局分析准确性差异 | 经过测试发现，Gemini网页版大模型能够基本正确识别布局结构，但当前使用的本地大模型在布局分析准确性上仍有问题。这影响了整体布局分析的可靠性。 | 2025-10-13 | S1 严重 | P0 | 处理中 | 1. 使用相同的测试数据分别测试Gemini网页版和本地模型<br>2. 对比布局分析结果的准确性<br>3. 发现本地模型输出质量明显低于Gemini网页版 | 本地模型应能达到与Gemini网页版相近的布局分析准确性 | 本地模型输出的布局分析结果存在较多错误，影响整体转换质量 | 方案1：优化本地模型的提示词，增加更详细的指导；方案2：考虑切换到Gemini API；方案3：实现模型结果的后处理验证机制 | 不同LLM模型在布局分析任务上的表现存在显著差异，需要选择最适合的模型或优化提示词策略 | 本地模型能够达到与Gemini网页版相近的布局分析准确性 | |
