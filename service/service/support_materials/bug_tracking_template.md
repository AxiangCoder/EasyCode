...
| Bug-013 | 代码生成器忽略DSL中的Flex布局规则 | 2025-10-09 | S1 严重 | P0 | 新建 | 1. 使用一个包含 layout.type 为 'flex' 的 DSL 文件生成前端代码。<br>2. 检查生成的 TSX 组件代码。 | 包含 layout.type: 'flex' 的 div 应该生成包含 'display: "flex"', 'flexDirection: "..."' 等样式的代码。 | 生成的 style 对象中完全没有 flex 布局相关的样式，layout 对象中的 flex 规则被忽略。 | | |
| Bug-014 | Flex布局下的子节点未按视觉顺序排序 | 2025-10-09 | S2 一般 | P2 | 新建 | 1. 使用一个可被推断为Flex布局的Sketch文件进行转换。<br>2. 确保文件中图层顺序与视觉顺序不一致。<br>3. 检查输出的DSL文件。 | 在被推断为Flex布局的父节点下，其 children 数组中的节点应按照视觉顺序（从上到下，从左到右）排列。 | children 数组中的节点顺序与输入文件中的图层顺序保持一致，而非正确的视觉顺序，导致潜在的DOM结构问题。 | | |