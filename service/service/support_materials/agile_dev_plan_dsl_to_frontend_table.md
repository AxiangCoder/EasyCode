...
| 修复SKETCH-26: 页面级组件布局处理缺陷 | 修复页面级组件布局处理的两个问题：1)跳过布局分析导致layout字段为空；2)强制绝对定位导致布局逻辑矛盾 | 修改`_traverse_layer`方法，移除`is_root_page`的特殊处理，让页面级组件也经过布局分析，并根据布局类型智能选择定位方式，避免强制绝对定位 | 使用包含多个画板的Sketch文件转换，验证页面级组件的layout字段包含正确的布局类型且无布局逻辑矛盾 | 开发者A | ⏳ 待开始 |
