...
*   **子任务分解 (Breakdown)**:
    1.  在 `FrontendRenderer._render_node()` 中，为每个生成的 div 元素添加 `data-dsl-id` 属性。
...
*   **模板样式基础完善**:
    *   **目标**: 在项目模板中集成 CSS Reset 样式，确保跨浏览器一致性。
    *   **任务**: 
        *   在 `converter/project_templates/vite-react-ts-tailwind/src/` 目录下创建 `reset.css` 文件。
        *   在 `src/index.css` 中引入 `@import './reset.css';`。
        *   确保生成的代码在构建时正确加载 reset 样式。
    *   **验证**: 生成项目后检查 `src/index.css` 包含 reset 导入，且浏览器开发者工具中能看到 reset 样式生效。

*   **根元素样式完善**:
    *   **目标**: 为生成的根元素添加宽高 100% 属性，确保页面占满整个视口。
    *   **任务**:
        *   在 `FrontendRenderer._render_node()` 中，为根节点（页面级别）自动添加 `width: '100%', height: '100%'` 样式。
        *   确保根元素的样式优先级高于其他样式规则。
        *   处理可能的样式冲突，确保根元素样式生效。
    *   **验证**: 生成项目后检查根元素包含 `width: 100%, height: 100%` 样式，且页面占满整个视口。

*   **自定义属性注入**:
    *   **目标**: 为生成的 DOM 元素添加自定义属性，便于调试和元素识别。
    *   **任务**:
        *   在 `FrontendRenderer._render_node()` 中，为每个生成的 div 元素添加 `data-dsl-id` 属性。
        *   属性值优先使用节点的 `do_objectID`，如不存在则使用 `name` 字段。
        *   确保属性值经过适当的转义处理，避免特殊字符导致的问题。
    *   **验证**: 生成项目后检查页面源码，确认所有 div 元素都包含正确的 `data-dsl-id` 属性。
